...
def existeGrupo(cid):
	...
	c.execute('SELECT COUNT(*) FROM Usuarios WHERE idGrupo =?', cid)
	try:
		for i in c:
...
def existeUserGru(uid,cid):
	...
	c.execute('SELECT COUNT(*) FROM Usuarios WHERE idUsuarioFK =? AND idGrupoFK =?', (uid, cid))
	try:
		for i in c:
				...
				if (EG == 0):
					...
					try:
						...
						if(EU == 0):
							...
							c.execute("INSERT INTO Usuarios (idUsuario,ALIAS,FC) VALUES (?,'@'?,?)", (uid, nocapital, fc))
		...
		if re.match(pattern, fc, flags=0):
			...
			if(EU == 1):
				try:
				  ...
				  c.execute("UPDATE Usuarios SET 'FC' = ?, 'ALIAS'='@'? WHERE idUsuario = ?", (fc, uname, uid)
			...
			try:
				...
				for i in c:
					...
					if i[0] is None:
						...
						c.execute('DELETE FROM Usuarios WHERE idUsuarioFK =?', uid)
						print("12")
